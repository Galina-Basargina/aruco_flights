...
# генерируем ключ id_galina для раздачи и уопирования его куда не попадя
cd ~/.ssh
...
git config user.name "Galina A. Basargina"
git config user.email gala.basargina@yandex.ru
...
# скачаем библиотеки openssl1.1 которые нужны python 3.6 и соберём их локально
...
CFLAGS=-I$(realpath ..)/openssl11/include \
  ...
  ./configure --enable-optimizations --without-doc-strings --prefix=$(realpath ..)/python36
# сборка python
make -j $(nproc)
